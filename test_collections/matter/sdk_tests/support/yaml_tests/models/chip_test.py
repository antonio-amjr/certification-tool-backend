...
class ChipTest(TestCase, UserPromptSupport, TestRunnerHooks, TestParserHooks):
    ...
    def parsing_start(self, count: int) -> None:
        pass

    def parsing_stop(self, duration: int) -> None:
        pass

    def test_parsing_start(self, name: str) -> None:
        pass

    def test_parsing_failure(self, exception: Exception, duration: int) -> None:
        self.current_test_step.record_error(
            f"Failed parsing with error: {str(exception)}"
        )

    def test_parsing_success(self, duration: int) -> None:
        pass

    # TestRunnerHooks methods
